...
def _config(max_steps: int = 5) -> RuntimeConfig:
    cfg = RuntimeConfig()
...
# 功能：验证缺少 API key 时 run 失败为事件状态而不是抛 SystemExit
# 设计：不注入 provider 并清空环境变量，断言 runner 捕获 provider 初始化失败并发布 failed
async def test_missing_provider_key_finishes_run_without_system_exit(
    ...
    runner = AgentRunner(_config(), extra_handlers=[_collect], runs_dir=tmp_path)

    outcome = await runner.run_and_capture("goal")

    finished = next(e for e in collected if e.type == "run.finished")  # type: ignore[attr-defined]
    assert outcome.status == "failed"
    assert outcome.reason == "llm_error"
    assert finished.status == "failed"  # type: ignore[attr-defined]
    assert finished.reason == "llm_error"  # type: ignore[attr-defined]
