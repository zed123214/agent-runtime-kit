...
from agent_runtime.core.runner import RunOutcome
from agent_runtime.core.session.manager import SESSION_CLOSED, SESSION_NOT_FOUND, SessionManager
...
async def test_provider_factory_is_lazy_for_non_llm_session_ops(tmp_path: Path) -> None:
    def fail_provider() -> object:
        raise AssertionError("provider should be lazy")
    ...
    manager = SessionManager(
        ...
        EventBus(),
        provider_factory=fail_provider,  # type: ignore[arg-type]
    )
