...
class AgentTuiApp(App[None]):
    """Agent Runtime Kit TUI：终端滚屏风格，实时展示 agent 执行过程。"""

    TITLE = "Agent Runtime Kit"
    BINDINGS = [
    ...
    _BANNER = (
        "[bold cyan]Agent Runtime Kit[/bold cyan]\n"
        "[dim]  daemon execution · typed IPC · tool permissions · MCP[/dim]\n"
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]Agent Runtime Kit[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]Agent Runtime Kit[/bold]  [dim]{self._host}:{self._port}[/dim]"
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update(f"[bold]Agent Runtime Kit[/bold]  [red]subscribe error: {e}[/red]")
            finally:
    ...
    def _handle_event_inner(self, event: dict[str, Any]) -> None:
        ...
        if t == "session.waiting_for_input":
            ...
            summary = _preview(description, 72)
            self._append(
                Static(
                    f"[dim]┌─[/dim] [cyan]{summary}[/cyan]  [dim]{short_id}[/dim]",
                    classes="log-line",
                )
            )
...
# TUI 入口：读取配置并启动 AgentTuiApp
def run(config: RuntimeConfig, replay_run_id: str | None = None) -> None:
    app = AgentTuiApp(config.host, config.port, replay_run_id=replay_run_id)
    app.run()
