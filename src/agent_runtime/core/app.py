...
from agent_runtime.core.runs import events_file, new_run_id
...
class CoreApp:
    ...
    async def _replay_events(
        ...
        if not path.exists():
            for candidate in (
                Path("~/.agentrt/sessions").expanduser().glob(f"*/runs/{run_id}/events.jsonl")
            ):
                path = candidate
                break
        if not path.exists():
    ...
    async def run(self) -> None:
        ...
        sessions_root = Path("~/.agentrt/sessions").expanduser()
