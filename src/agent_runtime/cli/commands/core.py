...
_PID_FILE = Path.home() / ".agentrt" / "agentrt-core.pid"


# 尝试连接 daemon，成功则正常返回，失败则抛出 ConnectionRefusedError/OSError
async def _ping_check(config: RuntimeConfig) -> None:
...
def cmd_core_status(config: RuntimeConfig) -> None:
    try:
...
def cmd_core_start(config: RuntimeConfig) -> None:
    try:
        asyncio.run(_ping_check(config))
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "agent_runtime.core"],
...
def cmd_core_stop(config: RuntimeConfig) -> None:
    pid = _running_pid()
    if pid is None:
        print("not running")
        return
